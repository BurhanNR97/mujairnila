...
public class Backpropagation {
    ...
    private final int neuron_output = 2;
    ...
    public void ReadData(ArrayList<modelSampel> list) {
        file_input = new double[list.size()][neuron_input];
        for (int i = 0; i < list.size(); i++) {
            file_input[i][0] = list.get(i).getM1();
            file_input[i][1] = list.get(i).getM2();
            file_input[i][2] = list.get(i).getM3();
            file_input[i][3] = list.get(i).getM4();
        }
...
//    }
    ...
    public void ReadTarget(List<Integer> list) {
        file_output = new int[list.size()];
        for (int i = 0; i < list.size(); i++) {
            file_output[i] = list.get(i);
        }
    }


    public void ReadDataTesting(double[][] list) {
        file_input = new double[list.length][neuron_input];
        for (int i = 0; i < list.length; i++) {
            for(int j=0; j<neuron_input; j++){
                file_input[i][j] = list[i][j];
            }
            max_data = file_input.length;
            mse = new double[max_epoch];
        }
    ...
    public void ReadTargetTesting(int[] list) {
        file_output = new int[list.length];
        for (int i = 0; i < list.length; i++) {
            file_output[i] = list[i];
        }
    }
    ...
    public void Belajar(ArrayList<modelSampel> sampel, List<Integer> indeksKode) {
        ReadData(sampel);
        ReadTarget(indeksKode);
        InisialisasiBobot();
        int epoh = 1;
        double sum_error;
...
//        } while (mse/MAX_SAMPLES > 0.022);
    ...
    public String Testing(double[][] dataUji) {
        //buat ngetest
        String hasil = "";
        ReadDataTesting(dataUji);
        //ReadTargetTesting(targetUji);
        double benar=0.0;
        double nol = 0.0;
        double satu = 0.0;
        ...
        /*double mNol = (nol/max_data)*100;
        double mSatu = (satu/max_data)*100;
        System.out.println("Nol  : "+(Math.round(nol)));
        System.out.println("Satu : "+(Math.round(satu)));
        System.out.println("Total Sample   : "+max_data);
        System.out.println("Akurasi Nol    : "+mNol+"%");
        System.out.println("Akurasi Satu   : "+mSatu+"%");*/
